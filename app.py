...
def api_health():
    ...
    # Check Internal API (self-check)
    try:
        response = requests.get('http://127.0.0.1:5000/api/status', timeout=5)
        health_status['Internal API'] = 'Online' if response.status_code == 200 else 'Offline'
    except Exception as e:
        logger.debug(f"Internal API check failed: {e}")
        health_status['Internal API'] = 'Offline'
    
    # Check Reddit API connectivity
    try:
        response = requests.get('https://www.reddit.com/.json', timeout=5)
        health_status['Reddit API'] = 'Online' if response.status_code == 200 else 'Offline'
    except Exception as e:
        logger.debug(f"Reddit API check failed: {e}")
        health_status['Reddit API'] = 'Offline'
    
    # Check Binance API connectivity
    try:
        response = requests.get('https://api.binance.com/api/v3/ping', timeout=5)
        health_status['Binance API'] = 'Online' if response.status_code == 200 else 'Offline'
    except Exception as e:
        logger.debug(f"Binance API check failed: {e}")
        health_status['Binance API'] = 'Offline'
    
    # Check News RSS Feed (Yahoo Finance)
    try:
        response = requests.get('https://finance.yahoo.com/news/rssindex/', timeout=5)
        health_status['News RSS Feed'] = 'Online' if response.status_code == 200 else 'Offline'
    except Exception as e:
        logger.debug(f"News RSS check failed: {e}")
        health_status['News RSS Feed'] = 'Offline'
    ...
    if openai_key:
        try:
            ...
            health_status['OpenAI API'] = 'Online' if response.status_code == 200 else 'Offline'
        except Exception as e:
            logger.debug(f"OpenAI API check failed: {e}")
            health_status['OpenAI API'] = 'Offline'
    else:
        health_status['OpenAI API'] = 'Not Configured'
    ...
    if telegram_token:
        try:
            ...
            health_status['Telegram Bot'] = 'Online' if response.status_code == 200 else 'Offline'
        except Exception as e:
            logger.debug(f"Telegram Bot check failed: {e}")
            health_status['Telegram Bot'] = 'Offline'
    else:
        health_status['Telegram Bot'] = 'Not Configured'
    ...
    health_status['Overall Status'] = overall_status
    health_status['Last Checked'] = last_checked
    health_status['Health Percentage'] = f"{(total_online / total_services * 100):.1f}%" if total_services > 0 else "0%"
